...
function ArcadeHome({onHome,onChoose}){return <section className="pranav-arcade"><ShelfBackdrop3D/><button className="text-button" onClick={onHome}><ArrowLeft/> Back to playroom</button><div className="pranav-arcade-hero"><span>🎮</span><div><div className="eyebrow">PRANAV’S MATH ARCADE</div><h1>Pick a world to play</h1><p>18 kinds of math play. Every round changes, so there is always a fresh mission.</p></div></div><div className="pranav-arcade-grid">{GAMES.map(([id,name,icon,tag])=><button key={id} onClick={()=>onChoose(id)}><span>{icon}</span><div><b>{name}</b><small>{tag}</small></div><i>PLAY ▶</i></button>)}</div></section>}
export function PranavMathArcade({onHome,voiceName}){const[id,setId]=useState(null),[q,setQ]=useState(null),[message,setMessage]=useState(''),[wins,setWins]=useState(0),[round,setRound]=useState(0);const start=game=>{setId(game);setQ(challenge(game));setMessage('');setRound(n=>n+1)};const answer=value=>{if(String(value)===String(q.answer)){ding(true);speakFriendly('Great thinking!',{voiceName});setMessage('Great thinking! You found it.');setWins(n=>n+1)}else{ding(false);setMessage('Good try. Look at the story and try again.')}};if(!id)return <ArcadeHome onHome={onHome} onChoose={start}/>;return <section className="pranav-arcade-play"><Confetti3D fireKey={wins}/><button className="text-button" onClick={()=>setId(null)}><ArrowLeft/> All arcade games</button><div className="arcade-play-top"><span>{GAMES.find(x=>x[0]===id)?.[2]}</span><div><div className="eyebrow">ROUND {round}</div><h1>{GAMES.find(x=>x[0]===id)?.[1]}</h1></div><button className="secondary" onClick={()=>start(id)}><RefreshCw/> New challenge</button></div><article className="arcade-question"><div className="arcade-art">{q.art}</div><h2>{q.prompt}</h2><div className="arcade-choices">{[...new Set(q.choices)].map(x=><button key={x} onClick={()=>answer(x)}>{x}</button>)}</div><p className={message.includes('Great')?'arcade-good':''} aria-live="polite">{message||'Tap an answer to play.'}</p>{message.includes('Great')&&<button className="primary jumbo" onClick={()=>start(id)}>Next challenge <Sparkles/></button>}</article></section>}
