...
export function speakFriendly(text,{voiceName='recorded:flo',rate=.9}={}){window.speechSynthesis?.cancel();if(activeAudio){activeAudio.pause();activeAudio=null}if(voiceName==='off')return;const clip=voiceName==='recorded:flo'&&naturalVoiceClips[text];if(clip&&typeof Audio!=='undefined'){const audio=new Audio(clip);activeAudio=audio;audio.addEventListener('ended',()=>{if(activeAudio===audio)activeAudio=null},{once:true});audio.play().catch(()=>{});return audio}/* Dynamic questions do not have a pre-rendered clip. Keep the button useful with the best English voice installed on the device. */speakDevice(text,{voiceName:voiceName==='recorded:flo'?undefined:voiceName,rate});}
