...
function values(mode,random){
 if(mode==='decimals'){const start=1+Math.floor(random()*8);return Array.from({length:8},(_,i)=>{const value=start/10+i/10;return {label:value.toFixed(1),value};});}
 if(mode==='geometry'){const start=12+Math.floor(random()*6)*2;return Array.from({length:8},(_,i)=>{const value=start+i*2;return {label:`${value} cm`,value};});}
