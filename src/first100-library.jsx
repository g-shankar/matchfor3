...
import FlipCard3D from './three-fx/FlipCard3D.jsx'
...
function CardViewer({ shelf, progress, update, onBack, onQuiz }) {
  ...
  const cardLabel = wordLabel(word)
  const cardNumber = shelf.id === 'concepts' && /^\d+$/.test(cardLabel) ? Number(cardLabel) : 0
  const cardImage = word?.image || word?.photo || `/first100/${shelf.id}/${wordSlug(word)}.jpg`
  return (
      ...
      <div className="f100-card-wrap" onTouchStart={(event) => { touchStart.current = event.changedTouches[0].clientX }} onTouchEnd={(event) => {
        ...
        <article className="f100-card">
          <FlipCard3D
            ...
            number={cardNumber}
            label={cardLabel}
            flipKey={wordKey(shelf, word)}
            fallback={<Photo shelf={shelf} word={word} />}
          />
