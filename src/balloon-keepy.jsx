...
export function BalloonKeepy({voiceName,onSave,onExit}){
  ...
  const bump=e=>{e.stopPropagation();if(!running){start();return}const next=hits+1,nextStreak=streak+1,nextBest=Math.max(best,nextStreak);motion.current.vy=-285;motion.current.vx+=(Math.random()-.5)*75;stats.current={...stats.current,hits:next,best:nextBest};setHits(next);setStreak(nextStreak);setBest(nextBest);if(next%5===0){setColor(c=>(c+1)%balloons.length);setMessage(`${next} taps! Amazing balloon keeping!`);speakFriendly('Amazing! Keep it up!',{voiceName})}else setMessage(`${next}! Keep it up!`)};
