...
Use the child selector in the header to switch profiles. Each child can choose and save one of five color themes independently. Shivani’s existing 50-skill math progress remains at the top level for backward compatibility. Pranav’s separate `preschool` record includes 40 early-learning skills across counting, shapes and colors, patterns, sorting, sound play, everyday sequencing, creative thinking, and spatial puzzles. A free creative studio adds drawing, mirror painting, stamps, shape collage, and color mixing. Balloon Keepy-Uppy adds a no-failure movement game with spoken counting celebrations. Wiggly Snake Garden adds touch, direction-button, and keyboard steering; a constant set of colorful shapes grows the player snake while three moving computer snakes can shorten it without ending play. Tap counts, collected shapes, and best runs appear only in the grown-up view. His visits can contain four, six, or eight large, visual activities with gentle retries, stars, and a separate 12-item toy shelf. Seventy-seven bundled warm-story voice recordings cover every preschool prompt and common encouragement, avoiding dependence on the browser’s synthetic default; installed device voices remain optional. His Parent view uses age-appropriate language and does not treat inconsistent preschool answers as an assessment. Both profiles share the same private browser/Firestore backup but never share attempts, adaptive evidence, sessions, themes, or rewards. The selected child is remembered only on the current browser. When Shivani is selected, a dismissible grown-up prompt requests her date of birth once. The date is stored inside the private browser/Firestore backup to support future age-aware learning, appears as an age in her profile label, and is excluded from parent emails. It is included in a parent-initiated JSON progress export.
